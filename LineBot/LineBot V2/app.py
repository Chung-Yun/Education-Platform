...
def feature_proposal(event, topic):
    ...
    for i in range(max_features):
        ...
        features_lst.append(            
            PostbackAction(
                    label=feature_name,
                    display_text= feature_name + '已選取', # output as messages
                    data='action=select_feature&'+str(event.source.user_id)+'&'+feature_name
                )
        )
...
def handle_message(event):
    ...
    # Section I: Interaction with users
        ...
        if my_message.find("我想學 ")==0:
            ...
            if table_name == 'Python':
                line_bot_api.reply_message(event.reply_token, TextSendMessage(text=table_name+ '已創建'))
            else:
                line_bot_api.reply_message(event.reply_token, TextSendMessage(text="開始創建 "+table_name))
                push_back_message(event,"正在幫您爬找資料") # There is technical issue here, might need to change
    ...
    # Section II: Backend setup for devs
        ...
        if setup_order.find('create')==0:
            ...
            line_bot_api.reply_message(event.reply_token, TextSendMessage(text=table_name+' has been reset'))
            if table_name=='replies':
                db.resetTableReplies()
            else: 
                db.resetTableComments(table_name)
            push_back_message(event,"done")
